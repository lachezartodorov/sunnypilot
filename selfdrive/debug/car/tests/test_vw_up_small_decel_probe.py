from openpilot.selfdrive.debug.car.vw_up_small_decel_probe import (
  ACC_SYSTEM_ADDR,
  BUS,
  EXPECTED_COUNTER_ZERO_DATA,
  build_active_small_decel_acc_system,
)


def test_active_small_decel_acc_system_is_frozen():
  assert build_active_small_decel_acc_system() == (ACC_SYSTEM_ADDR, EXPECTED_COUNTER_ZERO_DATA, BUS)
...
def test_active_small_decel_counter_and_checksum_advance():
  from opendbc.can import CANPacker

  packer = CANPacker("vw_pq")
  frames = [build_active_small_decel_acc_system(packer)[1] for _ in range(16)]
